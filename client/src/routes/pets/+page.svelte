<script lang="ts">
	...
	// Derive last visit date from visits array
	function getLastVisitDate(pet: PetResponse): string | null {
		if (!pet.visits?.length) return null;
		return pet.visits
			.map((v) => v.date)
			.sort()
			.at(-1) ?? null;
	...
	let filteredPets = $derived(() => {
		...
		return pets.filter(
			(pet) =>
				...
				getLastVisitDate(pet)?.includes(query)
		);
	});
...
<div class="container mx-auto px-4 py-8">
	...
	<div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		...
		<Button href="/owners" class="gap-2">
			<Plus class="h-4 w-4" />
			Add Pet
		</Button>
	</div>
	...
	<div class="mb-6">
		<div class="relative max-w-md">
			...
			<Input
				type="search"
				placeholder="Search by name, type, owner, or last visit..."
				...
				<Table.Body>
					{#each filteredPets() as pet (pet.id)}
						<Table.Row class="hover:bg-muted/50">
							<Table.Cell>
								<a href="/owners/{pet.ownerId}/pets/{pet.id}" class="font-medium text-foreground hover:text-primary">
									{pet.name}
								</a>
							</Table.Cell>
							<Table.Cell>
							...
							<Table.Cell>
								<span class="text-sm text-muted-foreground">{pet.birthDate ?? '--'}</span>
							</Table.Cell>
							<Table.Cell>
							...
							<Table.Cell>
								<span class="text-sm text-muted-foreground">{getLastVisitDate(pet) ?? '--'}</span>
							</Table.Cell>
							<Table.Cell>
